...
function baum_welch_log_multiple_sequences!(
    ...
    K = length(obs_sequences)
    T = [length(obs_sequences[k]) for k in 1:K]
    (; logα, logβ, logγ, logξ) = multiple_fb_log_storage
    ...
    @progress for iteration in 1:max_iterations
        for k in 1:K
            # Local forward-backward
            update_obs_logdensity!(obs_logdensities[k], obs_sequences[k], hmm, par)
            logL_by_seq[k] = forward_backward_log!(
        ...
        # Aggregated transitions
        @views p0 = reduce(+, exp.(logγ[k][:, 1]) for k in 1:K)
        P = reduce(+, dropdims(sum(exp, logξ[k]; dims=3); dims=3) for k in 1:K)
        p0 ./= sum(p0)
        P ./= sum(P; dims=2)
...
function baum_welch_log_multiple_sequences(
    ...
    multiple_fb_log_storage = initialize_forward_backward_log_multiple_sequences(
        obs_logdensities
    )
    result = baum_welch_log_multiple_sequences!(
        obs_logdensities,
        multiple_fb_log_storage,
        obs_sequences,
        hmm_init,
        par;
        max_iterations=max_iterations,
